...
class attachment_act extends act
{
    ...
    function attachment_js_action()
    {
        ...
        if (!rank::arcget($aid, $this->view->usergroupid, 'down')) {
            $link = "<a id='att' title='$name' href='javascript:alert(\"" . lang('without_authorization_can_ not_download') . "\");'>$name</a>";
            echo tool::text_javascript($link);
            exit;
        } else {
            if (config::get('verifycode')) {
                $link = "<a target='_blank' title='$name' id='att' href='" . url::create('attachment/downfile/aid/' . $aid . '/v/ce') . "'>$name</a>";
                echo tool::text_javascript($link);
                exit;
            } else {
                $link = "<a target='_blank' title='$name' id='att' href='" . url::create('attachment/down/aid/' . $aid) . "'>$name</a>";
                echo tool::text_javascript($link);
                exit;
            }
        }
