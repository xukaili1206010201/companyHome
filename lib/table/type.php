...
class type extends table
{
    ...
    {
        if ($act == 'modify')
            return 'typeid,parentid,typename,subtitle,htmldir,template,listtemplate,showtemplate,ishtml,isshow,ispages,includecatarchives,linkto,htmlrule,listhtmlrule,showhtmlrule,description,keyword,stype,thumb,thumb_width,thumb_height';
        else return 'typeid,typename,subtitle,htmldir,isshow,ispages,linkto,stype';
    }
    ...
    static function gethtmlrule($typeid, $tag = 'listhtmlrule')
    {
        ...
        $default = array(
            'listhtmlrule' => '{$caturl}/type_{$page}.html',
            'showhtmlrule' => '{$caturl}/show_{$aid}(_{$page}).html',
        );
        if (isset($default[$tag])) return $default[$tag];
    }
