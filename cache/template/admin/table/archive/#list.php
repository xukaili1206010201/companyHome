...
<div class="blank5"></div>
            ...
            <input type="submit" value="搜索" name="submit"  onclick="this.form.action='<?php echo url::modify('table/'.get('table').'/type/search');?>'" class="btn btn-steeblue search-btn" />
        </form>
</div>
<div class="linebreak"><div class="blank10"></div></div>
<div class="col-xs-12 col-sm-6 col-md-6 col-lg-6 text-right">

<input type="button" value="添加内容" onClick="javascript:location.href='<?php echo $base_url;?>/index.php?case=table&act=add&table=archive&admin_dir=<?php echo get('admin_dir');?>'"  class="btn btn-primary" />
<input type="button" value="审核内容" onclick="javascript:window.location.href='<?php echo url::create('table/list/table/archive/needcheck/1');?>'"  class="btn btn-steeblue" />
<input type="button" value="回收站" onclick="javascript:window.location.href='<?php echo url::modify("table/".get('table')."/deletestate/1/page/1");?>'"  class="btn btn-navy" />
</div>
...
foreach($data as $d) { ?>
            <tr>
                ...
                <td class="sort" align="center" class="table_input_c"><?php echo form::input("listorder[$d[$primary_key]]",$d['listorder'],'class="input_c"');?></td>

                <td class="sort" align="center"><?php echo cut($d['aid']);?></td>
                <td class="htmldir" align="center" style="width:100px;"><span class="hotspot" onmouseover="tooltip.show('查看内容所属栏目');" onmouseout="tooltip.hide();"><a href="<?php echo url("archive/list/catid/".$d['catid'],false);?>" target="_blank"><?php echo catname($d['catid']);?></a></span></td>
                <td align="left" class="catname"><?php echo cut($d['title']);?></td>
                <td class="htmldir" align="center"><?php if(!empty($d['attr1'])) { ?><span onmouseover="tooltip.show('<?php echo attr1($d['attr1']);?>');" onmouseout="tooltip.hide();"><?php echo helper::yes($d['checked']);?></span><?php } ?></td>
                <!-- <td align="center"><?php echo cut($d['username']);?></td> -->
                <td class="htmldir" align="center"><?php echo cut($d['view']);?></td>
                <td class="htmldir" align="center"><?php echo cut($d['adddate']);?></td>
                <td class="htmldir" align="center"><?php 
if((config::get('isecoding') == 1 && $d['isecoding'] == '0') || $d['isecoding']=='1'){
echo $d['ecoding'];	
}
?></td>
                <td class="htmldir" align="center"><?php echo helper::yes($d['checked']);?></td>
