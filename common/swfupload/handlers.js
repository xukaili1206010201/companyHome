...
function uploadSuccess(file, serverData) {
    try {
        ...
        progress.toggleCancel(false);
        //getEditor('content').InsertHtml('<p><img src="'+serverData+'"/></p>');
		$(".box_nav > a")[1].click();
		$("#image_select > option:last").attr('selected',true);
		$("#image_show").load('index.php?case=table&act=add&table=archive&admin_dir=admin&site=default&ajax='+$("#image_select > option:last").val()); 
		//document.getElementById("box_con_1").style.display = 'block';
		//document.getElementById("box_con_0").style.display = 'none';
		//document.getElementById("box_con_2").style.display = 'none';
		//document.getElementById("image_select").selectedIndex = document.getElementById("image_select").options.length-1;
    } catch (ex) {
        this.debug(ex);
    }
}
